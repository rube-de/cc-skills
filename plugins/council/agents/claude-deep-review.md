...
- **Overly broad exception handlers**: Is `except Exception:` or `catch (e) {}` too broad? Does it swallow `asyncio.CancelledError`, `TypeError`, `KeyboardInterrupt`, or specific errors that need different handling? Does the handler log enough detail to diagnose the root cause?
