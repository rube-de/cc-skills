...
4. Create PR via `gh pr create` with plan summary as description. Derive `BRANCH=$(git branch --show-current | tr '/' '-')`; if `.claude/$BRANCH/.cdt-issue` exists, include `Closes #N` in the PR body.
