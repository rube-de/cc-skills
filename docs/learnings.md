...
**Good pattern**: Embed a `<!-- dlc-reply:{database_id} -->` HTML comment sentinel in every DLC reply body. On re-runs, the script filters authentic sentinel-bearing replies (authored by PR author / viewer / bot) from the reviewer inventory, and the skill matches by author + `database_id` for reliable "already replied" detection, preventing raw substring forgeability by third-party reviewer comments.
