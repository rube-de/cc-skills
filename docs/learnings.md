...
**Mitigation (no flag exists): sandbox the cwd.** *Project-level* discovery is keyed to omp's working directory, so run omp from a throwaway `mktemp -d` dir (lands outside the repo) and attach the real content by **absolute** `@path`. Verified empirically against omp v16.1.1: a probe `.omp/tools/probe.ts` that writes a marker executed under `omp -p --no-tools` when omp ran inside its dir, and did **not** execute when omp ran from a separate sandbox cwd with the file attached by absolute path.

**The cwd sandbox is partial — it does not cover user-level tools.** A follow-up Codex P2 (verified against v16.1.7) showed that `discoverCustomToolPaths` → `loadCapability({cwd})` always sets `ctx.home = os.homedir()`, and `loadTools` scans `getUserClaude(ctx)/tools` (`~/.claude/tools`) plus `~/.omp/plugins/*` — both keyed to `$HOME`, not cwd. So a clean cwd starves the untrusted repo's *own* tools (the unconditional import-time RCE, the vector that matters most) but leaves user-level tools loaded and model-callable: `--no-tools` only empties built-in `toolNames`, and `--no-extensions` gates custom *commands* (`sdk.ts:1949`), not custom *tools* (`sdk.ts:1767`, ungated). `os.homedir()` *does* honor `$HOME` (so `HOME=$sandbox` would starve user-level discovery too), but omp reads its auth/model config from `~/.omp/agent/`, so a bare `HOME` relocation breaks the session. Robust isolation for fully-untrusted review is therefore OS-level (container / dedicated clean-home account), not a flag or env tweak. Lesson: a "sandbox" that closes one discovery root can silently leave a sibling root (home-keyed vs cwd-keyed) open — enumerate *every* root the loader reads before calling the mitigation complete.
...
| Trying to fix the row above with a per-run *random* heredoc delimiter instead of eliminating the heredoc | Bash never parameter-expands the delimiter *word* itself when matching the heredoc terminator — this holds for both `<<"$DELIM"` and `<<$DELIM`; quoting only controls whether the *body* gets expanded, not what closes the heredoc. Confirmed empirically (bash 3.2, arm64-darwin) with a marker-file test: in both forms the heredoc terminates on a line matching the literal source text `$DELIM`, never the variable's runtime value — so "randomizing" `$DELIM`'s value never reaches the terminator-matching logic at all, in either form | There is no shell-native form that generates a per-run random delimiter — the terminator is always the fixed literal text baked into the script source, regardless of quoting. Don't reach for "randomize the delimiter" as a lighter-weight fix for the row above — go straight to the `Write`-tool fix, it's not more work |
...
| Fixing an untrusted-content shell risk in `--body-file "$FILE"` while leaving `--title "...{agent-composed text}..."` on the same command untouched | The body and the title are composed from the *same* untrusted source (raw scan output → agent-authored summary), but only the body went through a file; the title stayed a directly shell-interpolated double-quoted argument, which does not stop `$(...)`/backtick expansion. Two independent council reviews (PR/Issue #245) caught this on the same diff that had just fixed the body | When neutralizing untrusted content reaching a shell command, audit *every* argument on that command line, not just the one the original bug report named. A double-quoted `--flag "text"` argument is exactly as exposed as a heredoc if `text` embeds untrusted substrings — the fix is the same file-and-read-back pattern: `--flag "$(cat "$FILE")"` is safe because command substitution runs once and never re-parses its own captured output, unlike a heredoc delimiter which can be prematurely terminated by content that matches it |
| Deterministic scratch-file path derived from stable inputs (repo + checkout, or PR number) reused across every invocation | Two concurrent or sequential runs sharing the same formula resolve to the *same* directory/filename — one run's `Write` or cleanup can clobber another's in-flight draft, and failure-path drafts need a manual timestamp/PID suffix to avoid colliding with each other too (which itself only narrows a same-second window, it doesn't close it) | Use `mktemp -d` for the scratch directory instead of a formula — its randomness makes every invocation's directory unique, eliminating the collision class entirely rather than narrowing it. mktemp requests mode 0700 but that request is still masked by the caller's umask like any `mkdir` — verified empirically that `umask 777` (and `umask 700`) produce a mode-000 directory, not 0700 — so force it explicitly with `chmod 700` right after the `mktemp -d` call succeeds, don't trust mktemp alone. Because the path is no longer derivable by formula, thread it across separate Bash tool calls as agent-carried literal text (print it in the prep step, substitute the literal value into the post step) the same way `{rest_id}`/`{database_id}` placeholders already are — don't try to "recompute" a random value |
| `rm -f` (or any cleanup command) as the last statement in a success branch | If cleanup fails for an unrelated reason (permissions, directory already gone) after the real operation (e.g. `gh issue create`) already succeeded, the cleanup's own nonzero exit becomes the branch's exit status — a caller sees "failure" and may retry an operation that already completed, creating a duplicate | Capture the primary operation's success/failure into a variable *before* running cleanup, branch on that captured value, and make cleanup itself best-effort (`cleanup-cmd`, with a non-fatal cleanup note) so its exit code can never overwrite the already-determined result |
| Redacting sensitive `grep` output with a `sed` pass keyed on "first N colon-delimited fields = path:line" | Any filename containing a colon (valid on Linux/macOS) makes `grep -rn`'s own `path:line:` prefix ambiguous — the `sed` pattern silently fails to match, and the raw unredacted line (with the actual secret value) passes straight through. Verified empirically: `config:old.env:1:export AWS_KEY=AKIA...` leaked the full key | Don't try to reconstruct "redact everything after the Nth field" from a combined `path:line:content` string. Add `-o`/`--only-matching` to the `grep` call instead so only the matched token reaches output — safe here because none of the patterns captures the secret value itself, only a short fixed prefix/keyword (`AKIA`, `ghp_`, `password =`) |
| Threading a literal (unrecomputable) value like a `mktemp -d` path across a Bash → Write → Bash tool-call boundary, validated only for shape (non-empty, absolute) | An absolute-path check doesn't stop the value from containing shell metacharacters (`$()`, backticks) — if the environment's `$TMPDIR` contains them, `mktemp` preserves them verbatim, and the next Bash call re-embeds the printed value as literal source text in a double-quoted assignment (`VAR="{printed value}"`), which the shell re-expands at that call's execution time | When a value must be threaded as literal text into a *later* shell script (not just used as data inside `"$(cat "$FILE")"`, which is safe because it's read back through the argument, never re-embedded as source), validate its character set against an allowlist (e.g. `tr -d 'A-Za-z0-9/._-'` and reject anything left over) before ever printing it, not just its absolute-path shape |
| Composing a `dlc-pr-check` reply without the `<!-- dlc-reply:{database_id} -->` sentinel | The sentinel is the *only* thing the skill's "already replied" detection scans for on review bodies and issue comments (no thread/state link exists for them) — a reply that's substantively posted but missing the sentinel is invisible to that detection, so the same item reappears as "new" on every subsequent fetch. Reproduced directly: a real reply landed on GitHub but the very next cycle re-flagged the same review body as unhandled | Never post a review-body or issue-comment reply without the sentinel — treat it as part of the reply, not optional metadata. If a sentinel-less reply is found after the fact, don't post a duplicate: edit the existing comment (`gh api .../issues/comments/{id} --method PATCH -F body=@file`) to append the sentinel, preserving the original content |

> Sources for pitfalls table: [AGENTS.md](../AGENTS.md) (conventions section), [Plugin Authoring guide](PLUGIN-AUTHORING.md), [Claude Code Skills docs](https://code.claude.com/docs/en/skills), [PR #40](https://github.com/rube-de/cc-skills/pull/40), [PR #41](https://github.com/rube-de/cc-skills/pull/41), [PR #43](https://github.com/rube-de/cc-skills/pull/43), [Issue #59](https://github.com/rube-de/cc-skills/issues/59), [Issue #115](https://github.com/rube-de/cc-skills/issues/115), [PR #157](https://github.com/rube-de/cc-skills/pull/157), [PR #210](https://github.com/rube-de/cc-skills/pull/210), [PR #222](https://github.com/rube-de/cc-skills/pull/222), [Issue #173](https://github.com/rube-de/cc-skills/issues/173), [PR #246](https://github.com/rube-de/cc-skills/pull/246)
...
**Exception (Issue #245):** `security`/`quality`/`perf`/`test`/`pr-validity` later gained `Write` back — narrowly, for one purpose: composing an issue body that embeds untrusted raw tool output (see the heredoc-injection row in the pitfalls table below) without heredocing it. This doesn't reverse the rule above. `allowed-tools` is additive pre-approval, never a capability boundary (line ~1401) — these skills already had unrestricted `Bash`, which can write any file via shell redirection, so adding `Write` grants no new capability. It only removes a permission prompt on the one safe path for untrusted content. The "read-only intent" signal these skills communicate is still accurate: none of them writes to a repo file, only to their own scratch path under `${TMPDIR:-/tmp}`.
>
> Source: [Issue #245](https://github.com/rube-de/cc-skills/issues/245), [PR #246](https://github.com/rube-de/cc-skills/pull/246)
...
**Good pattern**: Embed a `<!-- dlc-reply:{database_id} -->` HTML comment sentinel in every DLC reply body. On re-runs, the script filters authentic sentinel-bearing replies (authored by PR author, viewer actor, or CI bot) from the reviewer inventory, and the skill matches by author + `database_id` for reliable "already replied" detection, preventing raw substring forgeability by third-party reviewer comments.
