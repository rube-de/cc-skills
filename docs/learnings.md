...
| Deterministic scratch-file path derived from stable inputs (repo + checkout, or PR number) reused across every invocation | Two concurrent or sequential runs sharing the same formula resolve to the *same* directory/filename — one run's `Write` or cleanup can clobber another's in-flight draft, and failure-path drafts need a manual timestamp/PID suffix to avoid colliding with each other too (which itself only narrows a same-second window, it doesn't close it) | Use `mktemp -d` for the scratch directory instead of a formula — verified (GNU coreutils and BSD/macOS) that it's mode 0700 regardless of umask, so no separate `umask 077` call is needed, and its randomness makes every invocation's directory unique, eliminating the collision class entirely rather than narrowing it. Because the path is no longer derivable by formula, thread it across separate Bash tool calls as agent-carried literal text (print it in the prep step, substitute the literal value into the post step) the same way `{rest_id}`/`{database_id}` placeholders already are — don't try to "recompute" a random value |
